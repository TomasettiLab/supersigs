...
The `get_signature` function has two parameters: an input data frame `dt` and the `factor` (e.g. `factor = "Smoking"`). `dt` is a data frame with the following columns:
...
The process of converting a VCF file to this format is covered in [Preparing your data]. An example for `dt` is printed below.

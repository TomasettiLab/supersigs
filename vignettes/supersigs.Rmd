...
The method uses single-base mutations in exomic data from cancer samples. Specifically, it requires data on every sample's mutations, the positions of those mutations, and the age of all patients in your study. This data can be represented as a list of mutations (e.g. in a VCF file). Below is an example dataset stored and accessible in the `supersigs` R package.
...
The `get_signature` function has two parameters: an input data frame `dt` and the `factor` (e.g. `factor = "Smoking"`). `dt` is a data frame with columns of counts for all 96 trinucleotide mutations, a `sample_id` for every patient, their `age`, and an `IndVar` column indicating whether they were exposed to the `factor` or not. The process of converting a VCF file to this format is covered in [Preparing your data]. An example for `dt` is printed below.
...
```{r}
# get_signature(dt = input_dt, factor = "Smoking") # Doesn't work right now, likely because input_dt has too few samples
```

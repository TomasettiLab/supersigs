...
Alternatively, you can use `readVcf` from the `VariantAnnotation` package to read in a VCF file as a VCF object. Then use `process_vcf` to transform the VCF object into the format required by the `supersigs` R package. Each row in `input_dt` corresponds to a different patient and the values in the columns are the number of mutations for each trinucleotide mutation.

```{r}
library(VariantAnnotation)

fl <- system.file("extdata", "chr22.vcf.gz", package="VariantAnnotation")
vcf <- readVcf(fl, "hg19") # VCF object
process_vcf(vcf)
```
